...
import { X, ChevronRight, LayoutDashboard, Wallet, TrendingUp, Brain, GraduationCap, Zap } from 'lucide-react';
...
export const StoryOnboarding: React.FC<StoryOnboardingProps> = ({ isOpen, onClose, onSelectLevel }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
      <div className="relative w-full max-w-[400px] aspect-[9/16] overflow-hidden rounded-[2.5rem] bg-[#050913] border border-white/10 shadow-2xl">
        ...
        <button 
          onClick={onClose}
          className="absolute top-10 right-6 z-20 p-2 text-white/50 hover:text-white transition-colors"
        >
          <X size={24} />
        </button>
        ...
        <AnimatePresence mode="wait">
          {current === -1 ? (
            ...
            >
              <div className="mb-8 flex h-20 w-20 items-center justify-center rounded-3xl bg-indigo-500 text-white shadow-2xl">
                <GraduationCap size={40} />
              </div>

              <h2 className="mb-4 text-3xl font-bold tracking-tight text-white leading-tight">
                Welcome to UseMoney AI
              </h2>
              ...
              <div className="w-full space-y-4">
                <button
                  ...
                  className="w-full flex items-center justify-between p-6 rounded-3xl bg-white/5 border border-white/10 text-left hover:bg-white/10 transition-all group"
                >
                  <div>
                    <h3 className="font-bold text-white text-lg">Beginner</h3>
                    <p className="text-sm text-slate-400">Simple language & guided UI</p>
                  </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between p-6 rounded-3xl bg-indigo-500/10 border border-indigo-500/20 text-left hover:bg-indigo-500/20 transition-all group"
                >
                  <div>
                    <h3 className="font-bold text-indigo-400 text-lg">Advanced</h3>
                    <p className="text-sm text-slate-400">Deep metrics & raw data</p>
                  </div>
                  <Zap size={20} className="text-indigo-400 group-hover:scale-110 transition-transform" />
                </button>
              </div>
            </motion.div>
          ) : (
            ...
            >
              <div className={cn(
                "mb-8 flex h-24 w-24 items-center justify-center rounded-3xl text-white shadow-2xl",
                story?.color
              )}>
              ...
              <p className="mb-12 text-lg text-slate-400 leading-relaxed font-medium">
                {story?.subtitle}
              </p>

              <button
                onClick={next}
                className="group flex items-center gap-3 rounded-2xl bg-white px-8 py-4 text-sm font-bold text-black hover:bg-slate-200 transition-all active:scale-95 shadow-lg shadow-white/10"
              >
                {story?.action}
